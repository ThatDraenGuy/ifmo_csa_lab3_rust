...
Статические данные располагаются в памяти процессора. Динамические данные располагабтся на стэке (который, по своей сути, тоже находится в памяти процессора). Куча не поддерживается.

## Система команд

Машинное слово занимает 4 байта. Тип данных определяется только интерпретацией программиста - с точки зрения процессора все данные являются просто набором бит.
...
В файле с машинным кодом инструкции хранятся в формате [ron](https://github.com/ron-rs/ron). Для удобства программиста все инструкции хранятся вместе с дебаг-информацией, содержащей строку и символ исходного кода, соответствующего каждой инструкции, и адрес ячейки памяти, в которой инструкция хранится.

Во время работы программы (в рантайме) инструкции представляются иерархией struct'ов и enum'ов.
...
[Исходный код](./src/translator/mod.rs)
...
[Исходный код](./src/machine/datapath.rs)
...
[Исходный код](./src/machine/control_unit.rs)
